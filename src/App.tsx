...
function App() {
  ...
  const fetchPrimaryLibraries = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setMcpError("⚠️ Server backend belum tersedia. Fitur Pustaka menggunakan data lokal.");
      }
    } catch (err) {
      // Server not running — expected in static deployment
      setMcpError("⚠️ Mode offline — backend tidak aktif. Beberapa fitur terbatas.");
    }
  };
  ...
  const fetchMcpServers = async () => {
    try {
      ...
      // Backend not running — expected in static deployment
      // Don't spam console; user sees offline notice from fetchPrimaryLibraries
    }
  };
      ...
      <main className={`mx-auto relative ${isPhonePreview ? '' : 'max-w-6xl'}`} style={{ isolation: 'isolate' }}>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 p-5 rounded-2xl border border-indigo-500/20 relative overflow-hidden">
            ...
            {mcpData ? (
              <div className="space-y-4 text-sm relative z-10 animate-in fade-in">
               ...
               <div className="h-40 flex flex-col items-center justify-center text-slate-600 text-xs text-center border border-dashed border-slate-700 rounded-xl space-y-2">
                <p className="text-slate-500 font-medium">🎤 Cara Pakai E-Tahsin:</p>
                <p>1️⃣ Pilih Surah & Ayat → Klik <b className="text-purple-400">RAG</b> untuk memuat teks</p>
                <p>2️⃣ Klik tombol <b className="text-red-400">🎙️ Rekam</b> lalu bacakan ayat</p>
                <p>3️⃣ Klik <b className="text-red-400">⏹️ Stop</b> — AI akan evaluasi bacaan Anda</p>
              </div>
            )}
          </div>
          ...
          {(() => {
            ...
            const verseArabic = mcpData?.verifiedText || tahsinVerseArabic;
            const verseQiraat = verseArabic ? analyzeVerse(verseArabic, surahNum, verseNum) : null;
                  ...
                  {!verseArabic ? (
                    ...
                    <>
                      ...
                      <div className="bg-emerald-500/5 border border-emerald-500/30 rounded-lg p-3">
                        ...
                        <p className="text-right text-xl font-arabic text-emerald-200 leading-loose" dir="rtl">{verseArabic}</p>
                      ...
                      <div className="bg-slate-950/60 rounded-lg border border-purple-500/20 overflow-hidden">
                        ...
                        <div className="divide-y divide-slate-800/50">
                          {(() => {
                            const allReadings = verseArabic ? getAllReadings(surahNum, verseNum, verseArabic) : [];
                    ...
                    try {
                      ...
                      if (authResult) {
                        const profile: UserProfile = {
                          ...
                          displayName: authResult.user.displayName || authResult.user.email?.split('@')[0] || 'User',
                          tier: 'Reguler',
                          role: 'User',
                        ...
                        setGoogleToken(authResult.accessToken);
                        saveUserWithExpiry(profile);
                        addLog(`[Auth] Berhasil masuk dengan Google: ${profile.email}`);
                        setShowLoginModal(false);
                        setLoginEmail('');
                        setLoginPassword('');
                      }
                    } catch (err: any) {
                      setLoginError(`Google Sign-In gagal: ${err.message || 'Periksa konfigurasi Firebase atau koneksi internet.'}`);
                    } finally {
                      setLoginLoading(false);
                    }
                  }}
                  className="w-full py-3 bg-white hover:bg-slate-100 text-slate-800 font-bold rounded-xl transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 text-sm disabled:opacity-50"
                >
