...
export class QuranicaApiClient {
   ...
   */
  async sendChatMessage(messages: { role: string; content: string }[], mode: 'tafsir' | 'master' | 'maqashid' | 'genz' = 'tafsir', userTier?: string): Promise<{ reply: string }> {
    const res = await fetch(`${this.baseUrl}/api/chat`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ messages, mode, userTier: userTier || 'Reguler' })
    });
    if (!res.ok) {
      const ct = res.headers.get("content-type") || "";
      const errText = ct.includes("json") ? (await res.json()).error : await res.text().then(t => t.slice(0, 200));
